...
class fpwCategoryThumbnails {
	...
	function __construct( $path, $version ) {
		...
		$this->translationURL = 'http://svn.wp-plugins.org/fpw-category-thumbnails/translations/' . 
								$this->fctVersion . '/fpw-category-thumbnails-' . $this->fctLocale;
								
		//	set translation path (language pack)
		$this->languagepackPath = WP_CONTENT_DIR . '/languages/plugins/fpw-category-thumbnails-' . $this->fctLocale;
				...
				if ( 'ngg-' == substr( $value, 0, 4 ) ) {
					...
					if ( 'Author' === $value ) {
						...
						if ( wp_attachment_is_image( $value ) ) {
							...
							return '<img src="' . $fpwImage[0] . '" class="attachment-thumbnail size-thumbnail" alt width="' . get_option('thumbnail_size_w') . '">';
						} else {
							return 	'<span style="font-size: large; color: red">' .
	...
	function translationAvailable() {
		...
		if ( is_readable( $this->languagepackPath . '.mo' ) ) 
			return 'installed';

		//if ( is_readable( $this->languagepackPath ) ) 
			//return 'installed';

		if ( version_compare( $this->wpVersion, '3.6.0', '<' ) ) {
			$this->translationResponse = wp_remote_get( $this->translationURL . '.mo', array( 'timeout' => 300 ) );
		} else {
			$this->translationResponse = wp_safe_remote_get( $this->translationURL . '.mo', array( 'timeout' => 300 ) );
		}
	...
	function custom_print_footer_scripts() {
		...
		$pointerContent .= '<li>' . __( 'No changes', 'fpw-category-thumbnails' ) . '</li>';
		$pointerContent .= '</ul>';
    	?>
	...
	private function doGetLanguage() {
		if ( 'not_exist' == $this->translationStatus ) {
			$m = __( 'Language pack for this version is not yet available.', 'fpw-category-thumbnails' );
		} elseif ( 'installed' == $this->translationStatus ) {
			$m = __( 'Language pack is already installed.', 'fpw-category-thumbnails' );
		} else {
			$handle = @fopen( $this->languagepackPath . '.mo', 'wb' );
			fwrite( $handle, $this->translationResponse[ 'body' ] );
			fclose($handle);
			if ( version_compare( $this->wpVersion, '3.6.0', '<' ) ) {
				$this->translationResponse = wp_remote_get( $this->translationURL . '.po', array( 'timeout' => 300 ) );
			} else {
				$this->translationResponse = wp_safe_remote_get( $this->translationURL . '.po', array( 'timeout' => 300 ) );
			}
			if ( !is_wp_error( $this->translationResponse ) ) {
				$handle = @fopen( $this->languagepackPath . '.po', 'wb' );
				fwrite( $handle, $this->translationResponse[ 'body' ] );
				fclose($handle);
			}
			$this->translationStatus = 'installed';
			$m = __( 'Language pack downloaded successfully. It will be applied as soon as this page is reloaded.', 'fpw-category-thumbnails' );
		}
		return $m;			
	}
