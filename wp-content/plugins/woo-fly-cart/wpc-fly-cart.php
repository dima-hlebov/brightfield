...
! defined( 'WOOFC_VERSION' ) && define( 'WOOFC_VERSION', '2.2.3' );
					...
					if ( is_array( $items ) && ( count( $items ) > 0 ) ) {
						...
						foreach ( $items as $cart_item_key => $cart_item ) {
							if ( ! isset( $cart_item['bundled_by'] ) && apply_filters( 'woocommerce_widget_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
								...
								if ( ( $_link !== 'no' ) && ! empty( $_product_link ) ) {
									$cart_item_name = sprintf( '<a ' . ( $_link === 'yes_popup' ? 'class="woosq-btn" data-id="' . $_product_id . '"' : '' ) . ' href="%s" ' . ( $_link === 'yes_blank' ? 'target="_blank"' : '' ) . '>%s</a>', esc_url( $_product_link ), $_product->get_short_description() );
								} else {
									$cart_item_name = $_product->get_short_description();
								}
								...
								if ( $_product->is_sold_individually() || ! empty( $cart_item['woosb_parent_id'] ) || ! empty( $cart_item['wooco_parent_id'] ) ) {
